#include <bootloader_service/file.h>
#include <bootloader_service/memory.h>
...
EFI_STATUS openFile(IN EFI_FILE_HANDLE root, OUT EFI_FILE_HANDLE *file,
                    ...
                    IN UINT64 attribute) {
    return uefi_call_wrapper(root->Open, 5, root, file, fileName, mode,
                             attribute);
}
...
EFI_STATUS getFileInfo(IN EFI_FILE_HANDLE file,
    ...
    return uefi_call_wrapper(file->GetInfo, 4, file, informationType,
                             bufferSize, buffer);
}
...
void loadKernel(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    status = handleProtocol(ImageHandle, &gEfiLoadedImageProtocolGuid,
                            (VOID **)&loadedImage);
    status = handleProtocol(loadedImage->DeviceHandle,
                            &gEfiSimpleFileSystemProtocolGuid,
                            (VOID **)&volume);
    status = openVolume(volume, &root);
    status = openFile(root, &file, (CHAR16 *)L"kernel.bin",
    ...
    status     = allocatePool(EfiRuntimeServicesData, bufferSize,
                          (VOID **)&fileInfo);
    status     = getFileInfo(file, &gEfiFileInfoGuid, &bufferSize,
                         (VOID *)fileInfo);

    Print(L"\tFileName:%s\t FileInfoSize:%d\t FileSize:%d\t Physical "
          L"Size:%d\n",
          fileInfo->FileName, fileInfo->Size, fileInfo->FileSize,
          fileInfo->PhysicalSize);

    status =
        allocatePages(AllocateAddress, EfiConventionalMemory,
                      (fileInfo->FileSize + 0x1000 - 1) / 0x1000, &addr);
    ...
    closeProtocol(volume, &gEfiSimpleFileSystemProtocolGuid,
                  loadedImage->DeviceHandle, NULL);
    closeProtocol(loadedImage, &gEfiGraphicsOutputProtocolGuid,
                  ImageHandle, NULL);
}