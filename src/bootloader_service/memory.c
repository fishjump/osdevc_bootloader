...
EFI_STATUS allocatePool(IN EFI_MEMORY_TYPE poolType, IN UINTN bufferSize,
                        OUT VOID **buffer) {
    return uefi_call_wrapper(gBS->AllocatePool, 3, poolType, bufferSize,
                             buffer);
}
...
EFI_STATUS allocatePages(IN EFI_ALLOCATE_TYPE type,
    ...
    return uefi_call_wrapper(gBS->AllocatePages, 4, type, memoryType,
                             pages, addr);
}
...
EFI_STATUS getMemoryMap(IN OUT UINTN *memoryMapSize,
    ...
    return uefi_call_wrapper(gBS->GetMemoryMap, 5, memoryMapSize,
