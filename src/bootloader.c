...
#include <bootloader_service/vedio.h>
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE        ImageHandle,
    ...
    allocatePages(AllocateAddress, EfiConventionalMemory, 1,
    ...
    bootInfo->graphicInfo.frameBufferSize =
        graphicsOutputProtocol->Mode->FrameBufferSize;
    bootInfo->graphicInfo.horizontalResolution =
        graphicsOutputProtocol->Mode->Info->HorizontalResolution;
    bootInfo->graphicInfo.verticalResolution =
        graphicsOutputProtocol->Mode->Info->VerticalResolution;
    bootInfo->graphicInfo.pixelsPerScanLine =
    ...
    status = getMemoryMap(&memoryMapSize, memoryMap, &mapKey,
                          &descriptorSize, &desVersion);
    memoryMapSize += descriptorSize * 5;
    status = allocatePool(EfiRuntimeServicesData, memoryMapSize,
                          (VOID **)&memoryMap);
    ...
    struct MemoryDescriptor *descriptors =
        bootInfo->memoryInfo.descriptors;
    Print(L"Get EFI_MEMORY_DESCRIPTOR Structure:%018lx\n", memoryMap);
    for(int i = 0; i < memoryMapSize / descriptorSize; i++) {
        int                    memoryType = 0;
        EFI_MEMORY_DESCRIPTOR *descriptor =
            (EFI_MEMORY_DESCRIPTOR *)((CHAR8 *)memoryMap
                                      + i * descriptorSize);

        Print(L"MemoryMap %4d %10d (%16lx<->%16lx)\n", descriptor->Type,
              descriptor->NumberOfPages, descriptor->PhysicalStart,
              descriptor->PhysicalStart
                  + (descriptor->NumberOfPages << 12));

        if(descriptor->NumberOfPages == 0
           || descriptor->PhysicalStart < 0x100000) {
            continue;
        }
        ...
        descriptors[descriptorCount].length  = descriptor->NumberOfPages
                                              << 12;
